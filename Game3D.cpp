	...
	, m_volume(0.7)
{
	...
	g_pEffectEnemy = new EffectManager;
	...
	for (int i = 0; i < MAX_ENEMY; i++)
	{
		if (m_pEnemy[i] != nullptr)
		{
			...
			if (IsKeyTrigger(VK_SHIFT))
			{
				...
				{
					g_pEffectEnemy->AtkEffect(m_pEnemy[i]->GetPos().x + 2, m_pEnemy[i]->GetPos().y, m_pEnemy[i]->GetPos().z + 2);
					m_pEnemy[i]->Touch();
